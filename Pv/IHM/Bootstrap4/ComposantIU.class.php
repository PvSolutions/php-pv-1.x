	...
	if(! defined('PV_COMPOSANT_IU_BOOTSTRAP4'))
	{
		...
		class PvTableauDonneesBootstrap4 extends PvTableauDonneesHtml
		{
			...
			protected function RenduRangeeDonnees()
			{
				...
				{
					$libelleTriAsc = '<span class="text-muted fa fa-sort-up" title="'.htmlspecialchars($this->LibelleTriAsc).'"></span>' ;
					$libelleTriDesc = '<span class="text-muted fa fa-sort-down" title="'.htmlspecialchars($this->LibelleTriDesc).'"></span>' ;
					$libelleTriAscSelectionne = '<span class="fa fa-sort-up" title="'.htmlspecialchars($this->LibelleTriAsc).'"></span>' ;
					$libelleTriDescSelectionne = '<span class="fa fa-sort-down" title="'.htmlspecialchars($this->LibelleTriDesc).'"></span>' ;
		...
		class PvNav2TableauDonneesBootstrap4 extends PvNavigateurRangeesDonneesBase
		{
			...
			protected function ExecuteInstructions(& $script, & $composant)
			{
				...
				if($composant->RangeeEnCours > 0)
				{
					for($i=$this->MaxRangeesPrec; $i>0; $i--)
					{
						$rangeeEnCours = $composant->RangeeEnCours - ($i) ;
						if($rangeeEnCours <= 0)
						{
							continue ;
						}
				...
				$ctn .= '<li class="page-item"><a class="page-link" href="javascript:'.$composant->AppelJsEnvoiFiltres($paramRangee).'" title="'.($composant->RangeeEnCours + 1).'">'.($composant->RangeeEnCours + 1).'</a></li>'.PHP_EOL ;
				if($composant->RangeeEnCours < intval($composant->TotalElements / $composant->MaxElements))
				{
					$paramRangeeSuivante = array_merge($parametresRendu, array($composant->NomParamIndiceDebut() => ($composant->RangeeEnCours + 1) * $composant->MaxElements)) ;
					$ctn .= '<li class="page-item"><a href="javascript:'.$composant->AppelJsEnvoiFiltres($paramRangeeSuivante).'" class="page-link" title="'.$composant->TitreRangeeSuivante.'">'.($composant->RangeeEnCours + 1).'</a></li>'.PHP_EOL ;
					for($i=1; $i<$this->MaxRangeesSuiv && $i < $composant->TotalRangees; $i++)
					{
						$rangeeEnCours = $composant->RangeeEnCours + ($i + 1) ;
						if($rangeeEnCours >= $composant->TotalRangees - 1)
						{
							break ;
						}
